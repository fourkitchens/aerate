...
function getBrowser(results) {
  try {
    ...
    if (check.nonEmptyString(data.browser_name)) {
...
function mapChart(results, chart) {
  const filteredResults = results.filter(filterResults.bind(null, chart.view, chart.key, chart.derivative, chart.metric));
...
function mapResults(options, results) {
  ...
  Object.keys(budget).forEach((key) => {
    const barChartData = [];
    if (mapped[0].render.data.median.firstView[key] !== undefined) {
      const actualItem = {};
      // Measurement
      // Measurement Value
      actualItem.percentage = mapped[0].render.data.median.firstView[key].value;
      // Measurement Name
  ...
  return {
    ...
    results: mapped[0].render.data.median,
...
function map(options, results) {
  ...
  check.assert.nonEmptyString(options.location, 'invalid location option');
